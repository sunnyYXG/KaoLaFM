...
@implementation YXGDBModel


#pragma mark - override method
+ (void)initialize
{
    if (self != [YXGDBModel self]) {
        [self createTable];
    }
}
...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[YXGDBHelper dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
{
    //判断是否是JKBaseModel的子类
    for (YXGDBModel *model in array) {
        if (![model isKindOfClass:[YXGDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            YXGDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
